...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
    ...
    if (! igraph_create(&graph, &edges, 0, IGRAPH_UNDIRECTED)) {
        ...
        if (! multi) {
            ...
            igraph_vector_int_t* color = (igraph_vector_int_t* )malloc(sizeof(igraph_vector_int_t));
            igraph_vector_int_init(color, 10);
            check_err(igraph_automorphism_group(&graph, color, &generators, IGRAPH_BLISS_FS, &info));
